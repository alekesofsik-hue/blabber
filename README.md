...
| `/kb reindex [all\|doc_id]` | Переиндексировать уже сохранённые документы по текущим KB-флагам (`doc_id` видно в `/kb`) |
...
**Источники знаний:**
- Файлы: TXT, MD — встроено; PDF — нужен `pypdf`; DOC и DOCX — нужен `python-docx`
...
**Переиндексация:** `/kb reindex all` или `/kb reindex <doc_id>` пересобирает vector index по уже сохранённым chunk’ам. `doc_id` бот показывает в списке `/kb`. Это полезно после изменения KB-флагов или если нужно заново наполнить rollback buffer.
...
- **Save URL to KB** — сохраняет страницу в общую базу знаний пользователя
