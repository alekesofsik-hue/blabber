...
- ✅ **Балабол-новостник** (`/agent`) — агентный режим: поиск новостей по RSS и Hacker News
...
2. Каждое следующее сообщение обрабатывается агентным циклом:
...
3. Включить/выключить: `/agent on` / `/agent off`
...
│   ├── agent_tools.py           # Инструменты агента: RSS, HN, fetch URL
