...
def handle_memory(message):
    ...
    if len(parts) >= 2:
        ...
        if arg in ("on", "enable", "1", "true", "yes"):
            ...
            bot.send_message(
                ...
                "Сохранение всегда требует подтверждения кнопкой.",
            )
            return
        if arg in ("off", "disable", "0", "false", "no"):
            ...
            bot.send_message(
                ...
                "Ты всё равно можешь сохранять вручную через /remember и /prefer.",
            )
            return
    ...
    bot.send_message(
        ...
        "Факты проходят через ту же проверку на смысловые дубли, что и ручной <code>/remember</code>.\n\n"
        "<i>Команды:</i>\n"
        "/memory on — включить\n"
