...
START_MESSAGE_HTML_TEMPLATE = (
    ...
    "🧠 <b>Память</b>\n"
    "<code>/remember</code> — факты о тебе и твоих задачах (похожие формулировки не дублирую)\n"
    "<code>/prefer</code> — как мне отвечать · <code>/profile</code> — просмотр и удаление\n"
    ...
    "📚 <code>/kb</code> — база знаний; файл в чат, <code>/kb url</code>, <code>/kb reindex</code>\n"
    "🕵️ <b>Балабол-новостник:</b> <code>/agent</code> — статус и кнопки; "
    "<code>/agent on</code> — режим с поиском по RSS и Hacker News; "
    "<code>/agent kb https://...</code> — сохранить ссылку в KB; "
    "<code>/duel</code> или <code>/compare_headlines</code> — сравнить два свежих заголовка из "
    "<b>разных</b> лент (нужен <code>/agent on</code>)\n"
    "🏛 <b>История дня:</b> <code>/historyday</code> или <code>/history</code> — отдельный сценарий: "
    "факт дня, связанное изображение, вопросы по сохраненному контексту\n"
...
HELP_MESSAGE_HTML = (
    ...
    "<code>/mode single</code> или <code>/mode qa</code> — каждый запрос отдельно\n\n"
    "<b>История чата</b>\n"
    ...
    "<code>/remember</code> текст — факт о тебе, проекте или задаче\n"
    "Похожие формулировки стараюсь не дублировать: могу пропустить дубль или обновить существующую запись\n"
    ...
    "<code>/kb url https://...</code> — добавить веб-страницу\n"
    "<code>/kb reindex [all|id] [embeddings|summary|all] [dry-run]</code> — переиндексировать KB по сохранённым данным (id видно в <code>/kb</code>)\n"
    "Файл в чат (TXT, PDF, DOC, DOCX, MD) — индексация автоматически\n\n"
    ...
    "сохранение ссылки в KB, "
    "сравнение двух заголовков из разных лент)\n"
    "<code>/agent kb https://...</code> — явно сохранить ссылку в KB через agent-поток\n"
    ...
    "<code>/agent on</code>)\n\n"
    "<b>История дня</b>\n"
    "<code>/historyday</code> или <code>/history</code> — статус, подсказки и быстрые кнопки\n"
    "<code>/historyday help</code> / <code>/historyday status</code> — повторно показать карточку сценария\n"
    "<code>/historyday fact</code> [MM-DD | YYYY-MM-DD] — исторический факт дня\n"
    "<code>/historyday image</code> [дата или вопрос] — связанное изображение и анализ\n"
    "<code>/historyday context вопрос</code> — ответ по сохраненному контексту из LanceDB\n"
    "<code>/historyday memory</code> — посмотреть, что сохранено в памяти сценария\n"
